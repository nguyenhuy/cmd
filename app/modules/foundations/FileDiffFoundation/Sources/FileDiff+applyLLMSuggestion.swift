...
extension FileDiff {
  ...
  public static func getFileChange(applying searchReplacePattern: String, to content: String) throws -> FileChangeDiff {
    let newContent: String = try apply(searchReplacePattern: searchReplacePattern, to: content)
  ...
  ///
  /// - Parameters:
  ///   - replacements: An array of search/replace pairs to apply
  ///   - content: The original file content to apply the changes to
  /// - Returns: A FileChangeDiff representing the changes made
  public static func getFileChange(applying replacements: [SearchReplace], to content: String) throws -> FileChangeDiff {
    let newContent: String = try apply(searchReplace: replacements, to: content)
    return try getFileChange(changing: content, to: newContent)
  }
  ...
  public static func apply(searchReplacePattern diff: String, to fileContent: String) throws -> String {
    if diff.isEmpty {
      return fileContent
    }
    if !isLLMDiff(diff) {
    ...
    let changes = try diff.matches(of: llmDiffRegex).map { match in
...
//            firstFind = fileContent.index(fileContent.endIndex, offsetBy: -search.count - 1)
    ...
    return try apply(searchReplace: changes, to: fileContent)
  }
  ...
  ///   - fileContent: The original file content to apply the diff to
  /// - Returns: The modified file content with the diff applied
  public static func apply(searchReplace: [SearchReplace], to fileContent: String) throws -> String {
